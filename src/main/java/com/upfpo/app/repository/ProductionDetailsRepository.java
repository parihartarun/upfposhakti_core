...
public interface ProductionDetailsRepository extends JpaRepository<ProductionDetails, Integer>
{
	...
	/*@Query("select sum(p.actualProdcution) from ProductionDetails p join p.farmerProfile f where p.farmerId = f.farmerId and "
	...
	@Query("select sum(p.marketableSurplus) from ProductionDetails p join p.farmerProfile f where p.farmerId = f.farmerId and "
			+ "p.crop_id = :cropId and p.veriety_id=:cropVarietyId and p.season_id=:seasonId and f.fpoRefId=:masterId")
