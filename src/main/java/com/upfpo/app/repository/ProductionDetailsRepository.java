...
public interface ProductionDetailsRepository extends JpaRepository<ProductionDetails, Integer>
{
	...
	@Query("select sum(p.actualProdcution) from ProductionDetails p join p.farmerProfile f where p.farmerProfile.farmerId = f.farmerId and "
			+ "p.crop_id = :cropId and p.veriety_id=:cropVarietyId and p.season_id=:seasonId and f.fpoRefId=:masterId and p.financialYear = :financialYear and p.isDeleted = false")
	public Double getActualProductionbyFarmer(int cropId, int cropVarietyId, int seasonId, int masterId, String financialYear);
